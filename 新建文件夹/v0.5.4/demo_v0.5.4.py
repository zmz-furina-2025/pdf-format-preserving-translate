...
Rules:
1. Keep all math formulas and variables EXACTLY as they are. Do NOT translate or change them.
   - Variables like X, x, p_X, p(x) stay unchanged
   - Math expressions stay unchanged
   - Greek letters stay unchanged
...
class VectorPdfTranslator:
    ...
    def _tokenize_rich(self, text: str, bold_terms: List[str],
        ...
        # flags: bit0=bold, bit1=math(斜体公式), bit2=sup(上标), bit3=sub(下标), bit4=math_image(图片级平移)
        # 先识别图片级平移占位符 {{{数字}}}，标记 bit4
        tokens = []
        import re as _re_ph
        parts = _re_ph.split(r'(\{\{\{\d+\}\}\})', text)
        for part in parts:
            if not part:
                continue
            if _re_ph.match(r'^\{\{\{\d+\}\}\}$', part):
                tokens.append((part, 16))  # bit4 = math_image
            else:
                tokens.append((part, 0))
        # 合并 tokens（把相邻的同 flag tokens 合并）
        merged = []
        for t, fl in tokens:
            if merged and merged[-1][1] == fl and not (fl & 16):
                merged[-1] = (merged[-1][0] + t, fl)
            else:
                merged.append((t, fl))
        tokens = merged
        ...
        #    先识别译文中的 _X 下划线下标和 ^X 上标
        import re as _re_subsup
        new_tokens: List[Tuple[str, int]] = []
        for t, fl in tokens:
            if fl & 1 or fl & 2 or fl & 16:  # 粗体/公式/图片不处理
                new_tokens.append((t, fl))
                continue
            # 匹配 _X 下标和 ^X 上标（X 是单个字母或数字）
            parts = _re_subsup.split(r'(_([a-zA-Z0-9])|\^([a-zA-Z0-9]))', t)
            i = 0
            while i < len(parts):
                part = parts[i]
                if not part:
                ...
                if i + 2 < len(parts) and parts[i+1]:
                    if parts[i+2]:  # 下标 _X
                        new_tokens.append((parts[i], fl))
                        new_tokens.append((parts[i+2], fl | 8))  # bit3 = sub
                        i += 3
                    elif i + 3 < len(parts) and parts[i+3]:  # 上标 ^X
                        new_tokens.append((parts[i], fl))
                        new_tokens.append((parts[i+3], fl | 4))  # bit2 = sup
                        i += 4
                    else:
                        new_tokens.append((part, fl))
                        i += 1
                else:
                    new_tokens.append((part, fl))
                    i += 1
        tokens = new_tokens
        # 3) 标记上标（bit2）和下标（bit3）
        #    只匹配单字符数字/字母，避免误匹配
    ...
    def _draw_seg(self, page, x: float, y: float, seg: str,
        ...
        is_sup = bool(flags & 4)
        is_sub = bool(flags & 8)
        is_math_img = bool(flags & 16)
        # 图片级平移：遇到 {{{数字}}} 占位符，插入原文公式 PNG
        if is_math_img:
            import re as _re_ph
            ph_match = _re_ph.match(r'^\{\{\{(\d+)\}\}\}$', seg.strip())
            if ph_match:
                ph = f"{{{{{ph_match.group(1)}}}}}"
                if ph in self._math_images:
                    png_bytes, orig_w, orig_h = self._math_images[ph]
                    # 按当前字号缩放：高度按字号
                    scale = fs / (orig_h / 300 * 72) if orig_h > 0 else 1.0
                    new_w = (orig_w / 300 * 72) * scale
                    new_h = fs
                    rect = fitz.Rect(x, y - new_h * 0.8,
                                     x + new_w, y + fs * 0.2)
    ...
    def translate_page(self, page: fitz.Page) -> int:
        ...
        if not self._render_math:
            ...
            for blk in blocks:
                for line in blk.lines:
                    for s in line:
                        if s.is_math:
                            math_set.add(s.text.strip())
            math_list = list(math_set)
            for i, ms in enumerate(math_list):
                ph = f"{{{{{i}}}}}"
                math_map[ph] = ms
                # 渲染成 PNG
                for blk in blocks:
                    for line in blk.lines:
                        for s in line:
                            if s.is_math and s.text.strip() == ms:
                                try:
        ...
        for i, ab in enumerate(abbrev_set):
            ph = f"[[ABBR{i}]]"
            abbrev_map[ph] = ab
        # 关 render_math 时，公式内容不送翻译：送翻译前删掉
        math_map = {}
        math_images = {}  # ms → (png_bytes, orig_w, orig_h)
        if not self._render_math:
            # 把原文公式 span 渲染成 PNG
            self._math_images = {}
            for blk in blocks:
                    ...
                    for s in line:
                        if s.is_math:
                            try:
                                ...
                                png_bytes = pix.tobytes("png")
                                ms = s.text.strip()
                                self._math_images[ms] = (png_bytes, s.bbox[2] - s.bbox[0], s.bbox[3] - s.bbox[1])
                            except Exception:
                                pass
        ...
        def _restore_abbr(text):
            ...
            # 公式占位符保留，渲染时用 PNG 替换
            for ph, ms in math_map.items():
                # 不换回，保留占位符让 _tokenize_rich 识别成图片
                pass
            return text
        originals = [_apply_abbr(t) for t in originals]
        # 分块翻译：按占位符位置切段，每段单独翻译，再拼接
        import re as _re_ph
        all_segments = []  # 所有要翻译的纯文字段
        segment_map = []  # 记录每段属于哪句原文
        for sent_i, t in enumerate(originals):
            # 按 [[ABBR\d+]] 和 {{{\d+}}} 占位符切段
            parts = _re_ph.split(r'(\[\[\s*ABBR\s*\d+\s*\]\]|\{\{\{\s*\d+\s*\}\}\})', t)
            for part in parts:
                if not part:
                    continue
                # 是占位符就不翻译
                if _re_ph.match(r'^[\[\{\(\s\d\)\}\]]+$', part.strip()):
                    continue
                all_segments.append(part)
                segment_map.append((sent_i, part))
        # 批量翻译所有纯文字段
        translated_segments = self._cached_translate(all_segments)
        # 重新拼接
        translated = []
        seg_idx = 0
        for sent_i, t in enumerate(originals):
            ...
            for part in parts:
                ...
                if _re_ph.match(r'^[\[\{\(\s\d\)\}\]]+$', part.strip()):
                    ...
                    new_t += translated_segments[seg_idx]
                    seg_idx += 1
            translated.append(new_t)
        # 翻译后：占位符换回原文
