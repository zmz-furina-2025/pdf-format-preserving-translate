...
Rules:
1. Keep all math formulas wrapped in \\(...\\) LaTeX format. Use STANDARD LaTeX commands:
   - Sum: \\(\\sum_{{i=1}}^n\\) NOT Σ
   - Integral: \\(\\int_a^b f(x) dx\\) NOT ∫
   - Greek letters: \\(\\alpha\\), \\(\\beta\\), \\(\\gamma\\), \\(\\sigma\\)
   - Subscripts: \\(x_{{1}}\\), \\(p_X(x)\\)
   - Superscripts: \\(x^{{2}}\\), \\(P^{{\\perp}}\\)
