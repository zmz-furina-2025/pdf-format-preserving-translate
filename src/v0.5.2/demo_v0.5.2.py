...
# --------------------------------------------------------------------------- #
# LaTeX 公式渲染：用 matplotlib 渲染成透明 PNG
# --------------------------------------------------------------------------- #
def _render_math_png(latex: str, fontsize: float = 12.0, color=(0, 0, 0)) -> tuple[bytes, float, float]:
    """把 LaTeX 公式渲染成透明 PNG，返回 (png_bytes, width_pt, height_pt)"""
    import io
    import matplotlib
    ...
    buf = io.BytesIO()
    # math_to_image 需要公式包在 $...$ 里
    math_to_image(f'${latex}$', buf, dpi=300, format='png')
    buf.seek(0)
    img = Image.open(buf)
    # 转成 RGBA 确保透明背景
    if img.mode != 'RGBA':
        img = img = img.convert('RGBA')
    # 把白色背景变成透明
    data = img.getdata()
    new_data = []
    for item in data:
    ...
    # 计算宽高（pt）：300dpi 下，1pt = 1/72 inch = 300/72 像素
    w_px, h_px = img.size
    w_pt = w_px * 72 / 300
    h_pt = h_px * 72 / 300
    return png_bytes, w_pt, h_pt
...
class VectorPdfTranslator:
    ...
    def _render_math_to_png(self, latex: str, fontsize: float = 12.0) -> tuple[bytes, float, float]:
        """用 matplotlib 把 LaTeX 公式渲染成透明 PNG"""
        import io
        import matplotlib
        matplotlib.use('Agg')
        from matplotlib.mathtext import math_to_image
        ...
        try:
            ...
            png_bytes = out.getvalue()
            w_px, h_px = img.size
            return png_bytes, w_px, h_px
        except Exception as e:
            print(f"[math render error] {latex}: {e}")
            return None, 0, 0
            ...
            if bold:
                ...
                if not self._render_math:
                    ...
                    if png_bytes:
                        ...
                        cur_x += new_w
                        continue
                # 希腊字母（Σ α β γ）用中文字体渲染，避免 helv 缺字符
                has_greek = any(ord(ch) > 0x370 and ord(ch) < 0x3FF for ch in part)
                if has_greek:
