...
class QwenVLTranslator:
    """本地 Qwen2.5VL 视觉语言模型（通过 Ollama HTTP API 调用）。
    用截图识别公式 → LaTeX，比纯文本模型公式识别更好。"""
    def __init__(self, source: str = "en", target: str = "zh",
                 model: str = "qwen2.5vl:7b", host: str = "http://localhost:11434"):
        self._source = source
        self._target = target
        self._model = model
        self._host = host

    def recognize_formula(self, page: fitz.Page, bbox: Tuple[float, float, float, float]) -> str:
        """把公式区域截图，用 qwen2.5vl 识别成 LaTeX"""
        import base64
        import io
        ...
        try:
            resp = requests.post(
                ...
                json={
                    "model": self._model,
                    "prompt": "请把图片中的数学公式转换成 LaTeX 格式，仅输出 LaTeX 代码，不要任何解释文字。",
                    "images": [img_b64],
                    "stream": False,
            ...
            print(f"[qwen-vl] formula recognized: {result}")
            return result
        except Exception as e:
            print(f"[qwen-vl] formula recognition failed: {e}")
            return ""
...
Text: {t}"""
            try:
                ...
                resp.raise_for_status()
                result = resp.json().get("response", "").strip()
                print(f"[qwen-vl] in: {s[:60]}")
                print(f"[qwen-vl] out: {result[:80]}")
                out.append(result if result else t)
            except Exception as e:
                print(f"[qwen-vl] translation failed: {e}")
                out.append(t)
        return out
...
class VectorPdfTranslator:
    def __init__(self, translate_fn: TranslatorFn, target_lang: str = "zh",
        ...
        self._formula_recognizer = formula_recognizer  # 公式识别函数（page, bbox）→ LaTeX
    ...
    def translate_page(self, page: fitz.Page) -> int:
        ...
        # 如果有公式识别器，把公式区域截图，用 qwen2.5vl 识别成 LaTeX
        if self._formula_recognizer:
            # 收集所有 math span
            all_math_spans = []
            for blk in blocks:
                for line in blk.lines:
...
def main(argv: List[str]) -> int:
    ...
    ap.add_argument("--engine", choices=["mock", "tencent", "qwen", "qwen-vl"], default="mock")
    ...
    if args.engine == "tencent":
        ...
        formula_recognizer = None
    elif args.engine == "qwen-vl":
        vl = QwenVLTranslator("en", args.dst_lang)
        fn = vl.translate_batch
        formula_recognizer = vl.recognize_formula
    else:
